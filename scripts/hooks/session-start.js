...
function getSessionRetentionDays() {
  const raw = process.env.CT_SESSION_RETENTION_DAYS;
...
function isSessionStartContextDisabled() {
  const raw = String(process.env.CT_SESSION_START_CONTEXT || '').trim().toLowerCase();
  return ['0', 'false', 'off', 'none', 'disabled'].includes(raw);
}

function getSessionStartMaxContextChars() {
  const raw = process.env.CT_SESSION_START_MAX_CHARS;
...
function limitSessionStartContext(additionalContext, maxChars = getSessionStartMaxContextChars()) {
  ...
  const marker = '\n\n[SessionStart truncated context. Set CT_SESSION_START_MAX_CHARS to raise the cap or CT_SESSION_START_CONTEXT=off to disable injected context.]';
...
async function main() {
  ...
  if (explicitContextDisabled) {
    log('[SessionStart] Additional context injection disabled by CT_SESSION_START_CONTEXT');
  } else if (maxContextChars === 0) {
    log('[SessionStart] Additional context injection disabled by CT_SESSION_START_MAX_CHARS=0');
  }
  ...
  if (shouldInjectContext) {
    ...
    // Check for learned skills and inject their summaries
