...
def _resolve_homunculus_dir() -> Path:
    ...
    if override:
        ...
        print(
            f"[continuous-learning-v2] CLV2_HOMUNCULUS_DIR={override!r} is not absolute; ignoring",
            file=sys.stderr,
        )
    ...
    if xdg:
        if Path(xdg).is_absolute():
            return Path(xdg) / "claude-toolkit-homunculus"
        print(
            f"[continuous-learning-v2] XDG_DATA_HOME={xdg!r} is not absolute; ignoring",
            file=sys.stderr,
        )

    return Path.home() / ".local" / "share" / "claude-toolkit-homunculus"
...
def _update_registry(pid: str, pname: str, proot: str, premote: str) -> None:
    ...
    try:
        ...
        registry[pid] = {
            ...
            "root": proot,
            "remote": premote,
            "created_at": registry.get(pid, {}).get(
                "created_at",
