...
ANALYSIS_COOLDOWN="${CT_OBSERVER_ANALYSIS_COOLDOWN:-60}"
IDLE_TIMEOUT_SECONDS="${CT_OBSERVER_IDLE_TIMEOUT_SECONDS:-1800}"
...
analyze_observations() {
  ...
  if [ "${CLV2_IS_WINDOWS:-false}" = "true" ] && [ "${CT_OBSERVER_ALLOW_WINDOWS:-false}" != "true" ]; then
    echo "[$(date)] Skipping claude analysis on Windows due to known non-interactive hang issue (#295). Set CT_OBSERVER_ALLOW_WINDOWS=true to override." >> "$LOG_FILE"
    return
  fi
  ...
  MAX_ANALYSIS_LINES="${CT_OBSERVER_MAX_ANALYSIS_LINES:-500}"
  observer_tmp_dir="${PROJECT_DIR}/.observer-tmp"
  mkdir -p "$observer_tmp_dir"
  analysis_file="$(mktemp "${observer_tmp_dir}/ct-observer-analysis.XXXXXX.jsonl")"
  ...
  prompt_file="$(mktemp "${observer_tmp_dir}/ct-observer-prompt.XXXXXX")"
...
PROMPT
  ...
  timeout_seconds="${CT_OBSERVER_TIMEOUT_SECONDS:-120}"
  max_turns="${CT_OBSERVER_MAX_TURNS:-20}"
  exit_code=0
  ...
  CT_SKIP_OBSERVE=1 CT_HOOK_PROFILE=minimal claude --model haiku --max-turns "$max_turns" --print \
